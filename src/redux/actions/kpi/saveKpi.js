...
import { saveKpi, saveKpiPartial } from '../../../service/kpiPlanning';
...
export const actionSaveKpiPartial = (data, id) => async (dispatch) => {
  dispatch({
    type: SAVE_KPI,
    loading: true,
    status: null,
  ...
  try {
    const payload = await saveKpiPartial(data, id);
    if (payload?.data?.status_code === Success) {
      dispatch({
        type: SAVE_KPI_SUCCESS,
        loading: false,
